//---------------------------------------------------------------------------------------
//
// Sjofn: ELF object files linker for Odin ($Revision: 1.1.1.1 $)
//
...
// + generates comp and orb output format files
// + generates method table
//
// History bits:
// Sjofn  Attendant of Frigga. She was the goddess of affection, tenderness, and
//        sympathy. She stirs human to fall in love.
//
//---------------------------------------------------------------------------------------
...
int add_file( char *filename )
{
   ...
   else
   {
      ...
      {
         root_obj = file;
         DEBUG(( stderr, "add_file: linked in file '%s' as root_obj = %p\n", filename, file ));
      }
      else
      {
         object *p = root_obj;
         while( p->next ) p = p->next;
         p->next = file;
         DEBUG(( stderr, "add_file: linked in file '%s' as %p->next = %p\n", filename, p, file ));
      }
   }
